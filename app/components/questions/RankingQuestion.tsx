...
import { RankingQuestion as RankingQuestionType } from '../../types/survey';
import { FormattedTextEditor } from '../FormattedTextEditor';
import { Button } from '../ui/button';
import { GripVertical, LayoutGrid, PlusSquare, Trash2 } from 'lucide-react';
...
interface RankingQuestionProps {
  ...
  onChange: (question: RankingQuestionType) => void;
  onDelete: () => void;
  onDuplicate: () => void;
...
export function RankingQuestion({
  ...
  // Component for displaying a single ranking option
  const OptionRow = ({ id, text, index, isActive }: { id: string; text: string; index: number; isActive: boolean }) => (
    <div className="flex items-center w-full mb-2">
      <div className="basis-0 grow bg-white min-h-px min-w-px relative rounded-sm border border-black">
        <div className="relative size-full">
          <div className="box-border flex flex-row items-center justify-between px-4 py-2 w-full">
      ...
      {isEditing ? (
        ...
        <div className="mb-4">
          ...
          <div className="pl-8 mt-4 space-y-2">
            {question.options.map((option, index) => (
              <div key={option.id} className="flex items-center gap-2 p-2 bg-white border border-black rounded-sm">
                <span className="font-medium">{index + 1}.</span>
                <span>{option.text}</span>
                <GripVertical className="ml-auto size-5 text-gray-400" />
              </div>
            ))}
          </div>
        </div>
